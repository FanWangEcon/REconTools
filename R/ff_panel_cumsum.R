ff_panel_cumsum_grouplast <- function(df, svr_id, svr_x, svr_y, svr_cumsumtop = 'y_movingavg_lastestscores', stat='mean', quick=TRUE, verbose=FALSE) {
    ...
    #' print(df)
    #' quick <- TRUE
    #' ff_panel_cumsum_grouplast(df, svr_id='id', svr_x='x', svr_y='y', svr_cumsumtop = 'y_movingavg_lastestscore', stat='mean', quick=quick)
    #' quick <- FALSE
    #' verbose <- TRUE
    #' ff_panel_cumsum_grouplast(df, svr_id='id', svr_x='x', svr_y='y', svr_cumsumtop = 'y_movingavg_lastestscore', stat='mean', quick=quick, verbose=verbose)
    #' verbose <- FALSE
    #' ff_panel_cumsum_grouplast(df, svr_id='id', svr_x='x', svr_y='y', svr_cumsumtop = 'y_movingavg_lastestscore', stat='mean', quick=quick, verbose=verbose)
    #' ff_panel_cumsum_grouplast(df, svr_id='id', svr_x='x', svr_y='y', svr_cumsumtop = 'y_movingsum_lastestscore', stat='median')
    #' ff_panel_cumsum_grouplast(df, svr_id='id', svr_x='x', svr_y='y', svr_cumsumtop = 'y_movingsum_lastestscore', stat='sum')
    #' ff_panel_cumsum_grouplast(df, svr_id='id', svr_x='x', svr_y='y', svr_cumsumtop = 'y_movingsum_lastestscore', stat='min')
    #' ff_panel_cumsum_grouplast(df, svr_id='id', svr_x='x', svr_y='y', svr_cumsumtop = 'y_movingsum_lastestscore', stat='max')

    tb_data <- df %>% arrange(x)
    tb_data_cum_sum_top <- tb_data %>% mutate(!!sym(svr_cumsumtop) := 0)
    ar_latest_indi <- rep(NA, 1, length(unique(tb_data[['id']])))
    ...
    if (quick) {
        ...
        if (stat == 'sum'){
          for (row_ctr in seq(1, dim(tb_data)[1])) {
              ar_latest_indi[tb_data[['id']][row_ctr]] <- tb_data[['y']][row_ctr]
        ...
        for (row_ctr in seq(1, dim(tb_data)[1])) {
          ...
          if (verbose) {
            ...
            if (stat == 'sum'){
              ...
              fl_cum_sum_top_cur <- tb_data_up2row %>% summarize(y_sum_top_up2row = median(y)) %>% pull(y_sum_top_up2row)
            } else if (stat == 'max'){
              fl_cum_sum_top_cur <- tb_data_up2row %>% summarize(y_sum_top_up2row = max(y)) %>% pull(y_sum_top_up2row)
            } else if (stat == 'min'){
              fl_cum_sum_top_cur <- tb_data_up2row %>% summarize(y_sum_top_up2row = min(y)) %>% pull(y_sum_top_up2row)
            } else {
              warning('function stat not available')
            }
          } else {
            func = paste0(stat, '(y)')
            fl_cum_sum_top_cur <- tb_data[1:row_ctr,] %>% arrange(id, x) %>%
                group_by(id) %>% slice(n()) %>% ungroup() %>%
