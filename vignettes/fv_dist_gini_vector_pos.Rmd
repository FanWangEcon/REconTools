---
title: DIST GINI--Compute Gini Inequality Coefficient Given Data Vector (One Variable)
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{DIST GINI--Compute Gini Inequality Coefficient Given Sample (One Variable)}
...
This file works out how the [ff_dist_gini_vector_pos](https://fanwangecon.github.io/REconTools/reference/ff_dist_gini_vector_pos.html) function works from [Fan](https://fanwangecon.github.io/)'s *[REconTools](https://fanwangecon.github.io/REconTools/)* Package
...
See the formula below, note that when the sample size is small, there is a limit to inequality using the formula defined below given each $N$. So for small $N$, can not really compare inequality across arrays with different $N$, can only compare arrays with the same $N$. In another word, if 1 of N individual holds all resource, as $N$ increases, GINI will asymptote to 0, but it is very far away from 0 for low N.

# Formula

Not sure if this a standard way of computing GINI, but this is how I decided to compute this, the thought process is as follows, given monotonimcally increasing array $X$, with $x_1,...,x_N$:
...
I derive the formula in the steps below.
...
Given what we have from above, we obtain the gini formula
$$
 GINI =
  \left(\sum_{i=1}^N \sum_{j=1}^{i} x_j\right)
  \cdot
...
$$
 MostUnequalGINI\left(N\right) = \frac{2}{N+1}
$$
...
fv_dist_gini_vector_pos_test <- function(ar_pos) {
  ...
  it_n <- length(ar_pos)
  if (it_n <= 100)  warning('Data vector has n=',it_n,', max-inequality/min-gini=',2/(it_n + 1))
  # Sort
  ar_pos <- sort(ar_pos)
  # formula implement
...
cat('\nSmall N=2 Hard-Code, converge to 0.67, see formula above\n')
...
cat('\nSmall N=10 Hard-Code, convege to 0.18, see formula above\n')
